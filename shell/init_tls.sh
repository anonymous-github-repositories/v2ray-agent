...
# 恢复配置
resetNginxConfig(){
    `cp -Rrf /tmp/anonymous-github-repositories/nginx/nginx.conf /etc/nginx/nginx.conf`
    rm -rf /etc/nginx/conf.d/5NX2O9XQKP.conf
    echoColor green "\n恢复配置完毕"
}
# 备份
bakConfig(){
    mkdir -p /tmp/anonymous-github-repositories/nginx
    `cp -Rrf /etc/nginx/nginx.conf /tmp/anonymous-github-repositories/nginx/nginx.conf`
}
# 安装证书
installTLS(){
    ...
    ~/.acme.sh/acme.sh --installcert -d ${domain} --fullchainpath /tmp/anonymous-github-repositories/nginx/${domain}.crt --keypath /tmp/anonymous-github-repositories/nginx/${domain}.key --ecc >/dev/null
    if [[ -z `cat /tmp/anonymous-github-repositories/nginx/${domain}.key` ]]
    then
        echoColor red "证书key生成失败，请重新运行"
        resetNginxConfig
        exit
    elif [[ -z `cat /tmp/anonymous-github-repositories/nginx/${domain}.crt` ]]
    then
    ...
    echoColor green "证书目录/tmp/anonymous-github-repositories/nginx"
    ls /tmp/anonymous-github-repositories/nginx
...
init(){
    ...
    echoColor green "   12.Github[https://github.com/anonymous-github-repositories]"
    echoColor red "=============================="
